        ...
        Interceptor.attach(addrNewStringUTF, {
            onEnter: function (args) {
                if (args[1] != null) {
                    var module = Process.findModuleByAddress(this.returnAddress);
                    if (module != null && module.name.indexOf(so_name) == 0) {
                        var string = Java.vm.tryGetEnv.getStringUtfChars(args[1]).readUtf8String();
...
send("start");
send("enter the js file.");
setImmediate(function(){
    send("Hook start...");
    setTimeout(hook_libart,5000);
});
...
if __name__=="__main__":
    devices = frida.get_usb_device(3)
    pid = devices.spawn("/data/local/tmp/server")
    devices.resume(pid)

    session = devices.attach(pid)
    
    scripts = session.create_script(jscode)
    time.sleep(2)

    scripts.on("message",printMessage)
    scripts.load()
