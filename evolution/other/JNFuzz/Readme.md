...
```
export cc=~/Android/Sdk/ndk/android-standalone-toolchain-arm64/bin/aarch64-linux-android-gcc
```
...
```
$cc -o bubblesort bubblesort.c -ldl
...
./afl-fuzz -i in -o out -n -b 5 -- ./bubblesort
```

A one-minute run screenshot of one round of fuzzing：

![image-20250429125842108](Readme.assets/image-20250429125842108.png)

### 1.2 Experiment2

```
$cc -o harness1 harness1.c -ldl
...
./afl-fuzz -i in -o out -n -b 5 -- ./harness1
```

A one-minute run screenshot of one round of fuzzing：

![image-20250429130117224](Readme.assets/image-20250429130117224.png)

### 1.3 Experiment3

```
$cc -o server server.c lib/vm.c -I lib -L lib -Wall -O3 -ldl -Wl,--export-dynamic
...
./afl-fuzz -i in -o out -t 10000+ -b 5 -n -- ./server
```

A one-minute run screenshot of one round of fuzzing：

![image-20250429130259934](Readme.assets/image-20250429130259934.png)
...
```
$cc -o client client.c lib/spsc.c -I lib -lspsc -L lib -Wall -O3
...
./afl-fuzz -i in -o out -n -b 5 -- ./client
```

A one-minute run screenshot of one round of fuzzing：

![image-20250429125255794](Readme.assets/image-20250429125255794.png)
...
| ------------------- | ----------- | ----------- | ----------- | ----------- |
| total execs (10min) | 89.7k       | 2959        | 1472        | 88.9        |
| speed (exec/sec)    | 149.5       | 4.93        | 2.45        | 148.2       |

​	The results show that the execution speed of experiment 4 (i.e., JNFuzz module) is similar to that of the direct fuzzing of the binary file (148.2 execs/sec vs. 149.5 execs/sec), which is 30 times faster than experiment 2 (4.93 execs/sec) and 60 times faster than experiment 3 (2.45 execs/sec).
...
​	Experiment 3 required the initialization of the JVM before using dlopen and dlsym to fetch the Native functions. As a result, there was a significant decrease in performance (50.3%) compared to Experiment 1.
