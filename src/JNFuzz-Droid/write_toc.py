...
def gen_2demin(x=5, y=10, line="", num=0, totalsize=0):
    ...
    data = data + space + "jobjectArray par" + str(num + 1) + " = env->NewObjectArray(" + str(
        x) + " ,par" + str(num) + ",NULL);\n"
    ...
    data = data + space + "        par" + str(num + 2) + "[j] = getjni_" + types + "(data, " + str(btype_dict[
        ...
        y) + "+j+1)+" + str(totalsize) + ");\n"
    data = data + space + "    }\n"
    data = data + space + "    env->Set" + types.title() + "ArrayRegion(par" + str(num + 3) + ",0," + str(
        y) + ",par" + str(num + 2) + ");\n"
    data = data + space + "    env->SetObjectArrayElement(par" + str(num + 1) + ", i, par" + str(
        num + 3) + ");\n"
    data = data + space + "}"
    list.append(data)
...
def deal_basic_type(out_path, t, num, totalsize, size, apkname):
    ...
    if t == "java.lang.String[]":
        ...
        data = data + space + "    jstring par" + str(num + 4) + " = env->NewStringUTF(par" + str(
            num + 3) + ");\n"
        data = data + space + "    env->SetObjectArrayElement(par" + str(num + 1) + ",i,par" + str(
            num + 4) + ");\n"
        data = data + space + "    env->DeleteLocalRef(par" + str(num + 4) + ");\n"
        data = data + space + "}\n"
        list.append(data)
        list.append(", par" + str(num + 1))
        list.append("env->DeleteLocalRef(par" + str(num + 1) + ");")
        ...
        data = data + space + "jstring par" + str(num + 2) + " = env->NewStringUTF(par" + str(num + 1) + ");"
        list.append(data)
...
def analysis_line(out_path, line, num, size, totalsize, taintpath, apk_name):
    ...
    if line.split(":{is_tainted:")[1].split(",")[0] == "true":
        ...
        if "[][]" in line.split("type:")[1] and "[][][]" not in line.split("type:")[1]:
            print(" [*][*] Determines the dimensions of a two-dimensional array.")
...
def getc(out_path, apk_name, java_method, target_native_name, so_file, file_path, size):
    makefile(out_path, apk_name)
    pt = file_path.split("_")[-1][:-4]

    # read templeate
    f = open("./lib/examples/template.cpp", "r")
    lines = f.readlines()
    f.close()

    data, space = "", "    "
    f = open(out_path + "/outcome/" + apk_name + "/server" + pt + ".cpp", "w", encoding='utf-8')
    for line in lines:
