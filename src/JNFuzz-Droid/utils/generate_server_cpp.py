...
def getoverloading_jni_method(java_method, so_file, jni_method):
    parlist = java_method.split("(")[1].split(")")[0]
    parlist = parlist.replace("java/lang/String;", "java_lang_String_2").replace("[", "_3")
    jni_method_overloading = jni_method + "__" + parlist
    for met in so_file:
        if met == jni_method_overloading:
            return met


def getClass_Method(java_method, dirs, so_file):
    jni_method = java_method.split(":")[0].replace("_", "$").replace("/", "_").replace("$", "_1").replace(";.", "_")
    ...
    for met in so_file:
        if met == jni_method:
            return jni_method
    if flag == 0:
        jni_method_overloading = getoverloading_jni_method(java_method, so_file, jni_method)
        if jni_method_overloading is not None:
            return jni_method_overloading
        else:
            dynamic_method_name = get_dynamic_method_name(java_method, dirs)
            if dynamic_method_name != "None":
...
def deal_return(java_method):
    para_return = java_method.split(" ")[1]
    if para_return == "void":
        return ""
    else:
        return para_return
...
def deal_info(list):
    jni_method = list.split("*")[1].split("_")[0]
    jni_info = jni_method + "(env,"
    if list.split("*env,")[1].split(" ")[0] == "jobject":
...
def gen_2demin(x=5, y=10, line="", num=0, totalsize=0):
    ...
    list = []
    space = " " * 12
    data = "jclass par" + str(num) + " = env->FindClass(\"" + btype_dicts[types] + "\");\n"
    data += space + "jobjectArray par" + str(num + 1) + " = env->NewObjectArray(" + str(
        x) + " ,par" + str(num) + ",NULL);\n"
    data += space + "j" + types + " par" + str(num + 2) + "[" + str(y) + "];\n"
    data += space + "for(int i=0;i<" + str(x) + ";i++){\n"
    data += space + "    j" + types + "Array par" + str(
        num + 3) + " = env->New" + types.title() + "Array(" + str(y) + ");\n"
    data += space + "    for(int j=0;j<" + str(y) + ";j++){\n"
    data += space + "        par" + str(num + 2) + "[j] = getjni_" + types + "(data, " + str(btype_dict[
                                                                                                 types]) + "*(i*" + str(
        y) + "+j+1)+" + str(totalsize) + ");\n"
    data += space + "    }\n"
    data += space + "    env->Set" + types.title() + "ArrayRegion(par" + str(num + 3) + ",0," + str(
        y) + ",par" + str(num + 2) + ");\n"
    data += space + "    env->SetObjectArrayElement(par" + str(num + 1) + ", i, par" + str(
        num + 3) + ");\n"
    data += space + "}"
    list.append(data)
    list.append(",par" + str(num + 1))
    list.append("env->DeleteLocalRef(par" + str(num + 1) + ");")
...
def deal_basic_type(out_path, t, num, totalsize, size, apkname):
    ...
    if t == "java.lang.String[]":
        space = " " * 12
        data = "jclass par" + str(num) + " = env->FindClass(\"java/lang/String\");\n"
        data += space + "jobjectArray par" + str(num + 1) + " = env->NewObjectArray(" + str(
            array_length) + ", par" + str(num) + ", 0);\n"
        data += space + "for(int i=0;i<" + str(array_length) + ";i++){\n"
        data += space + "    char par" + str(num + 2) + "[" + size + "];\n"
        data += space + "    getjni_string(par" + str(num + 2) + ", data, i*10+" + str(
            totalsize) + ", " + size + ");\n"
        data += space + "    char * par" + str(num + 3) + " = par" + str(num + 2) + ";\n"
        data += space + "    jstring par" + str(num + 4) + " = env->NewStringUTF(par" + str(
            num + 3) + ");\n"
        data += space + "    env->SetObjectArrayElement(par" + str(num + 1) + ",i,par" + str(
            num + 4) + ");\n"
        data += space + "    env->DeleteLocalRef(par" + str(num + 4) + ");\n"
        data += space + "}\n"
        list.append(data)
        list.append(", par" + str(num + 1))
        list.append("env->DeleteLocalRef(par" + str(num + 1) + ");")
        ...
        space = " " * 12
        data += space + "getjni_string(par" + str(num) + ",data," + str(totalsize) + "," + str(size) + ");\n"
        data += space + "char *par" + str(num + 1) + " = par" + str(num) + ";\n"
        data += space + "jstring par" + str(num + 2) + " = env->NewStringUTF(par" + str(num + 1) + ");"
        list.append(data)
        list.append(", par" + str(num + 2))
        list.append("env->DeleteLocalRef(par" + str(num + 2) + ");")
        ...
        for m in btype_dict.keys():
            if m in t:
                ...
                data = data + space + switch_jni(m) + "Array par" + str(
                    num + 1) + " = env->New" + m.title() + "Array(" + str(array_length) + ");\n"
                data = data + space + "env->Set" + m.title() + "ArrayRegion(par" + str(
                    num + 1) + ", 0, " + str(
    ...
    if line.split(":{is_tainted:")[1].split(",")[0] == "true":
        ...
        if "[][]" in line.split("type:")[1] and "[][][]" not in line.split("type:")[1]:
            ...
            data = data + space + switch_jni(types) + "Array par" + str(
                num + 1) + " = env->New" + types.title() + "Array(" + str(nums) + ");\n"
            data = data + space + "env->Set" + types.title() + "ArrayRegion(par" + str(num + 1) + ", 0, " + str(
                nums) + ", par" + str(num) + ");\n"
            list.append(data)
            list.append(", par" + str(num + 1))
            list.append("env->DeleteLocalRef(par" + str(num + 1) + ");")
...
def generate_server_cpp(out_path, apk_name, java_method, target_native_name, so_file, file_path, size):
    ...
    for line in lines:
        if "typedef void function_t(JNIEnv *, jclass, jstring);" in line:
            ...
            data += space + "targetAppPath = \"/data/local/tmp/" + apk_name + ".apk" + "\";\n"
        elif "targetLibName = \"/data/local/tmp/libleak.so\";" in line:
            data += space + "targetLibName = \"/data/local/tmp/" + so_file + ".so" + "\";\n"
        elif "std::string className = \"org/arguslab/native_leak/MainActivity\";" in line:
            data += space + "std::string className=\"" + java_method.split(";")[0][1:] + "\";\n"
        elif "targetFunctionName = \"Java_org_arguslab_native_1leak_MainActivity_send\";" in line:
            if target_native_name == "unreachable":
                data += space + "targetFunctionName=\"" + java_method.split(":")[0].split(".")[1] + "\";\n"
            else:
                data += space + "targetFunctionName=\"" + target_native_name + "\";\n"
