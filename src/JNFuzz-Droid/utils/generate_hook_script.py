...
def generate_hook_script(target_native_name, so_file, file_path):
    if not os.path.exists("./utils_cmd/_agent.js"):
        log.error("[!] the _agent.js not exist.")

    log.info("[+] generate the agent.js file.")
    ...
    # get taint value
    taint_custom, taint_field, taint_type = "", "", ""
        ...
        for index, line in enumerate(lines):
            if "let so_files = \"libleak.so\";" in line:
                ...
                if taint_type == "java.lang.String":
                    ...
                    add_data += space * 6 + "send(` Taint: value:" + taint_custom + " addr:${args[" + str(
                        taint_field) + "]}`);\n"
