...
class Fuzz_and_taint_analysis:
    def __init__(self, name, out_path, arch, fuzz_one, fuzz_time, show_flag):
    ...
    def fuzz_apk_jni(self, so_graph, gcc, fuzz_one):
        ...
        if os.path.exists(apk_scripts):
            for i in os.listdir(apk_scripts):
                ...
                dynamic_dirs = os.path.join(self.out_path, "Dmethods", f"{self.apkname}_Dmethods")

                try:
                    target_native_name = getClass_Method(java_method, dynamic_dirs, so_graph.get(so_file))
                    totalsize = 0
    ...
    # for example {libegamepay,[Lcn/egame/terminal/paysdk/jni/EgamePayProtocol;.init:(Landroid/content/Context;)\
    # Lcn/egame/terminal/paysdk/jni/ProtocolMessage;]}
    def collect_jni_function(self):
        so_graphs = dict()
        ...
        if not judge_repeat_so(self.lib_path):
            all_so_files = self.add_other_arch()
        else:
    ...
    def fuzz_jni(self):
        so_graph = self.collect_jni_function()
