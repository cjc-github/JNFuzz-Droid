...
setImmediate(function() {
    ...
    Interceptor.attach(Module.findExportByName("libsdkutils.so", '__android_log_print'),{
        onEnter: function(args){
            send("log");
            //send(` __android_log_write ${args[1].readUtf8String()}`);
            //send(` __android_log_write ${args[2].readUtf8String()}`);
            //send(` __android_log_write ${args[3].readUtf8String()}`);
        },
    ...
    Interceptor.attach(Module.findExportByName(null, 'android_dlopen_ext'),{
        onEnter: function(args){
            ...
            if( library_path.includes("libsdkutils.so")){
